...
// const chrome = require('webextension-polyfill')
...
export default {
  ...
  methods: {
    onSubmit () {
      ...
      chrome.storage.sync.set({ envs: this.envs }).then(() => {
    ...
    uploadJsonChange () {
      ...
      reader.onload = function fileReadCompleted () {
        ...
        // clear
        this.$refs.jsonUploader.value = null
      }
      reader.readAsText(this.$refs.jsonUploader.files[0])
    },
    exportJson () {
      chrome.storage.sync.get(['envs']).then((result) => {
    ...
    mergeImport () {
      ...
      chrome.storage.sync.set({ envs: this.envs })
  ...
  mounted () {
    chrome.storage.sync.get(['envs']).then((result) => {
      ...
      if (result.envs === undefined) {
        this.envs = defaultEnvs
        chrome.storage.sync.set({ envs: defaultEnvs })
      }
    })
  }
}
